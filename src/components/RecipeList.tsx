    ...
    <div className="w-full grid md:grid-cols-2 xl:grid-cols-3 gap-5">
      {recipes.map((recipe) => (
        <Link key={recipe.id} to={`/recipe/${recipe.id}?query=${query}&page=${currentPage}`} className="flex">
          <div
            className="w-full p-[4px] bg-white rounded-[20px] flex flex-col justify-start items-center group"
          ...
          >
            ...
            <h2 className="p-2 pt-[12px] group-hover:text-green-500 transition-all duration-300 ease-in-out text-black text-center">
              {recipe.title}
            </h2>
          </div>
        </Link>
